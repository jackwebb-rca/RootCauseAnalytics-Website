  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
      ...
      <div className="relative bg-slate-100 aspect-[707/1000]">
        {slides.map((s, i) => (
          <Image key={s.src} src={s.src} alt={`RCA ${libraryName} Library sample: ${s.label}`}
            fill sizes="(min-width: 1024px) 560px, 100vw" priority={i === 0}
      ...
      <div className="flex items-center justify-center gap-1.5 px-5 py-3 border-t border-slate-200 bg-slate-50">
        {slides.map((s, i) => (
          <button key={s.src} type="button" onClick={() => setIdx(i)} aria-label={`Go to slide ${i + 1}: ${s.label}`}
            className={`h-1.5 rounded-full transition-all ${i === idx ? `w-6 ${activeDotClass}` : 'w-1.5 bg-slate-300 hover:bg-slate-400'}`} />
        ))}
      </div>
    </div>
  )
