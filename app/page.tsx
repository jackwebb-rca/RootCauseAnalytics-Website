import type { Metadata } from "next";
import Link from "next/link";
import Image from "next/image";
import FilmStrip from "@/components/FilmStrip";
import TrialTable from "@/components/TrialTable";
import evidence from "@/lib/evidence.json";

export const metadata: Metadata = {
  title: "Root Cause Analytics · Document extraction and synthetic data, Sydney",
  description:
    "RCA turns large quantities of PDFs and paper documents into clean, catalogued tables, and generates realistic synthetic Australian documents for AI training and evaluation. Sydney, Australia.",
  alternates: { canonical: "/" },
};

export default function HomePage() {
  const v2 = evidence.v2Benchmark;
  const trial = evidence.liveTrial;
  const trialDate = new Date(trial.date).toLocaleDateString("en-AU", {
    day: "2-digit",
    month: "short",
    year: "numeric",
  });

  return (
    <>
      <section className="hero hero-tiles">
        <h1 className="sr-only">
          Root Cause Analytics: document extraction and synthetic document
          generation software, Sydney
        </h1>
        <div className="cat-head rv-auto d1">
          <h2>Our catalogue</h2>
          <p className="lead">
            Root Cause Analytics builds two applications for document work:
          </p>
          <p>
            <b>RCA Document Library</b> reads and catalogues your real
            documents, providing a structured table from extracted fields.
          </p>
          <p>
            <b>RCA Document Generator</b> produces accurate, risk-free
            synthetic documents for AI training and testing in regulated
            sectors.
          </p>
        </div>
        <div className="tile-col">
          <div className="tile-stack">
            <div className="rv-auto d1">
              <Link className="prod tile" href="/document-library" aria-labelledby="tile-library">
                <span className="tile-head">
                  <span className="tn">№ 01</span>
                  <span className="tk">Catalogue documents and extract data</span>
                </span>
                <h3 id="tile-library">RCA Document Library</h3>
                <p>
                  Point it at your PDFs and scans. It reads them, extracts the
                  fields you care about, scores its own confidence on every
                  value, and builds a <b>clean table you can search and export</b>.
                </p>
                <ul>
                  <li>
                    {v2.accuracyPct}% field accuracy on OCR-only extraction (V2
                    benchmark, {v2.fields} fields)
                  </li>
                  <li>Offline-first: OCR, extraction, search and export without internet</li>
                  <li>Confidence built from four visible factors, not a black box</li>
                  <li>Optional AI validation layer that never overwrites OCR results</li>
                  <li>USD $0.10 per document extracted, no subscription or upfront fee</li>
                </ul>
                <span className="go">
                  Open the Document Library <i aria-hidden="true">→</i>
                </span>
              </Link>
            </div>
            <div className="rv-auto d2">
              <Link className="prod alt tile" href="/document-generator" aria-labelledby="tile-generator">
                <span className="tile-head">
                  <span className="tn">№ 02</span>
                  <span className="tk">Generate synthetic documents</span>
                </span>
                <h3 id="tile-generator">RCA Document Generator</h3>
                <p>
                  Realistic synthetic Australian documents for training and
                  evaluating AI, with <b>complete ground truth on every field</b>.
                  No real patient anywhere in the pipeline.
                </p>
                <ul>
                  <li>45 medical document types across 81 clinical case archetypes</li>
                  <li>Deterministic: same seed, byte-identical PDFs and ground truth</li>
                  <li>Insurance packs, including red-flag packs with planted inconsistencies</li>
                  <li>Fully offline: about 5,000 documents in 5 minutes, no LLM calls</li>
                  <li>Valid Medicare and provider formats, NSW addresses, real structure</li>
                </ul>
                <span className="go">
                  Open the Document Generator <i aria-hidden="true">→</i>
                </span>
              </Link>
            </div>
          </div>
        </div>

        <div className="specimen-wrap rv-auto d2">
          <div className="stamp">Synthetic · No PHI</div>
          <div className="doc">
            <Image
              src="/specimens/spec-treatment-plan-1.png"
              alt="Synthetic staged care plan generated by the RCA engine"
              width={620}
              height={877}
              priority
              style={{ width: "100%", height: "auto" }}
            />
            <span className="mk m1" aria-hidden="true" style={{ left: "26.59%", top: "12.9%", width: "17.8%", height: "1.71%" }} />
            <span className="mk m2" aria-hidden="true" style={{ left: "66.48%", top: "15.07%", width: "10.44%", height: "1.4%" }} />
            <span className="mk m3" aria-hidden="true" style={{ left: "26.59%", top: "20.75%", width: "8.9%", height: "1.4%" }} />
            <span className="mk m4" aria-hidden="true" style={{ left: "33.63%", top: "35.28%", width: "28.68%", height: "1.63%" }} />
            <div className="panel">
              <div className="panel-head">
                <span>Extracted fields</span>
              </div>
              <div className="phead" aria-hidden="true">
                <span>Field</span>
                <span>Extracted value</span>
                <span className="pc">Accuracy</span>
              </div>
              <div className="prow p1">
                <span className="f">patient_name</span>
                <span className="v">Sarah Rebecca Wright</span>
                <span className="c">0.99</span>
              </div>
              <div className="prow p2">
                <span className="f">medicare_no</span>
                <span className="v">6600 13384 1</span>
                <span className="c">0.98</span>
              </div>
              <div className="prow p3">
                <span className="f">plan_date</span>
                <span className="v">26/06/2026</span>
                <span className="c">0.97</span>
              </div>
              <div className="prow p4 warn">
                <span className="f">diagnosis</span>
                <span className="v">Bilateral knee OA</span>
                <span className="c">0.84</span>
              </div>
            </div>
          </div>
          <p className="spec-cap" style={{ marginTop: 16 }}>
            <span>
              Specimen AUS-TRN-0001, generated by the RCA Synthetic Document
              Generator
            </span>
            <Link href="/evidence">
              Published extraction trials: three models, 20 documents →
            </Link>
          </p>
        </div>
      </section>

      <FilmStrip />

      <section className="band">
        <Image
          className="mark"
          src="/brand/logo-mark-paper.png"
          alt=""
          width={500}
          height={500}
        />
        <div className="band-inner">
          <h2 className="rv">
            Numbers from our published benchmarks, <em>misses included.</em>
          </h2>
          <div className="stat rv d1">
            <span className="mono">Extraction</span>
            <b>{v2.accuracyPct}%</b>
            <span>
              field accuracy on OCR-only extraction. V2 benchmark, {v2.fields}{" "}
              fields, no AI assistance
            </span>
          </div>
          <div className="stat rv d2">
            <span className="mono">Generation</span>
            <b>45 + 81</b>
            <span>
              medical document types and internally consistent clinical case
              archetypes
            </span>
          </div>
          <div className="stat rv d3">
            <span className="mono">Speed</span>
            <b>5 min</b>
            <span>
              to generate about 5,000 documents, fully offline, with ground
              truth
            </span>
          </div>
        </div>
      </section>

      <section className="evidence">
        <div className="sec-head rv">
          <h2>Published live trials</h2>
          <span>Live trial, {trialDate}</span>
        </div>
        <div className="ev-grid">
          <div className="rv d1">
            <p>
              Most vendors quote a single accuracy number. We publish the full
              run log: every field on every document, including the ones the
              models got wrong.{" "}
              <b>When a model fails a document, it is scored zero and recorded
              in the results.</b>{" "}
              In this run, two dense flow sheets exceeded the output limit on
              Haiku 4.5. Both are counted as failures in the table.
            </p>
            <p className="foot">
              Run {trial.dataset} · {trialDate}, Sydney time. Opus 5 pass
              added 21 Aug 2026, same documents and ground truth.
              <br />
              Total API spend across the three model runs: A$
              {trial.totalSpendAud}
            </p>
            <p style={{ marginTop: 22 }}>
              <Link className="btn btn-ghost" href="/evidence">
                See the live trials →
              </Link>
            </p>
          </div>
          <div className="rv d2 table-scroll">
            <TrialTable />
          </div>
        </div>
      </section>

      <section className="founder">
        <Image
          className="mark"
          src="/brand/logo-mark-paper.png"
          alt=""
          width={500}
          height={500}
        />
        <div className="founder-inner rv">
          <blockquote className="founder-quote">
            <p>
              I started RCA because I had this problem myself and could not
              find a good solution on the market. Most businesses sit on
              thousands of PDFs and paper documents they cannot use. RCA
              extracts the key values, builds a clean table for cataloguing
              them, and gives you metrics and patterns that were locked in
              paper. RCA Document Library does what the standard digitisation
              vendors do, with far more capability and near-instant
              turnaround,{" "}
              <em>
                at a fraction of the cost of traditional outsourced
                digitisation.
              </em>
            </p>
            <p>
              We operate solely on a consumption-based pricing model, so users
              can try our apps without committing to a subscription or paying
              upfront fees.
            </p>
          </blockquote>
          <div className="sig">
            <span className="name">Jack Webb</span>
            <span className="role">
              Founder, Root Cause Analytics
              <br />
              Sydney, NSW
            </span>
          </div>
        </div>
      </section>

      <section className="cta">
        <div className="cta-card rv">
          <div>
            <h2>
              Request a <em>free preview pack.</em>
            </h2>
            <p>
              Request a free preview pack with real sample documents so your
              team can evaluate hands on.
            </p>
          </div>
          <div className="cta-actions">
            <Link className="btn btn-gold" href="/contact">
              Request a preview pack
            </Link>
            <Link className="btn btn-paper" href="/contact">
              Talk to Jack →
            </Link>
            <span className="mono-note">
              Replies within one business day, Sydney time.
            </span>
          </div>
        </div>
      </section>
    </>
  );
}
